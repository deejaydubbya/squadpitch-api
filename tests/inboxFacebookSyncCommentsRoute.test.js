// POST /api/v1/workspaces/:id/connections/FACEBOOK/sync-comments
//
// Manual-trigger endpoint for Facebook Page comment polling. Mirrors
// the Threads + YouTube sync-replies/comments routes.
//
...
// In-memory client + connection fixtures the prisma mock reads from.
const clients = new Map();
const connections = new Map(); // key: `${clientId}:${channel}` → row

// Track enqueue + inline-fallback calls for assertions.
const enqueueCalls = [];
let nextEnqueueBehavior = "ok"; // "ok" | "queue-unavailable" | "throw"
...
vi.mock("../middleware/auth.js", () => ({
  // Synthetic actor sub via x-test-sub so tests can model owner vs
  // cross-tenant requests without dragging in a real JWT.
  getAuth0Sub: (req) => req?.headers?.["x-test-sub"] ?? null,
  requireAuth: (_req, _res, next) => next(),
}));

vi.mock("../workers/facebookCommentPollerWorker.js", () => ({
  enqueueFacebookCommentPollForConnection: vi.fn(async (id) => {
...
vi.mock("../domains/inbox/inbox.facebookCommentPoller.service.js", () => ({
  pollFacebookCommentsForConnection: vi.fn(async (conn) => {
...
const CLIENT_ID = "client-fb-route-1";
const CONN_ID = "conn-fb-route-1";
...
function seedDefaults() {
  ...
  connections.set(`${CLIENT_ID}:FACEBOOK`, {
    id: CONN_ID,
    clientId: CLIENT_ID,
    channel: "FACEBOOK",
    status: "CONNECTED",
    externalAccountId: "fb-page-123",
  });
}
...
function buildApp({ withAuth = true } = {}) {
  ...
  if (withAuth) {
    // Mirror the production setup: requireAuth gate before the router.
...
const URL = `/api/v1/workspaces/${CLIENT_ID}/connections/FACEBOOK/sync-comments`;
...
describe("POST /workspaces/:id/connections/FACEBOOK/sync-comments", () => {
  ...
  it("returns 404 when no Facebook connection exists on the workspace", async () => {
    connections.delete(`${CLIENT_ID}:FACEBOOK`);
    const app = buildApp({ withAuth: false });
    const res = await request(app)
      .post(URL)
  ...
  it("returns 400 when the connection status is not CONNECTED", async () => {
    const row = connections.get(`${CLIENT_ID}:FACEBOOK`);
    connections.set(`${CLIENT_ID}:FACEBOOK`, {
      ...row,
  ...
  it("returns 400 when externalAccountId is missing", async () => {
    ...
    const res = await request(app)
      ...
      .send({});
    expect(res.status).toBe(400);
    expect(res.body?.error).toBe("NO_PAGE_ID");
    expect(enqueueCalls).toHaveLength(0);
  });
  ...
  it("happy path: 202 + queued shape, enqueue called exactly once", async () => {
    ...
    expect(res.body.message).toMatch(/Facebook comment sync queued/i);
