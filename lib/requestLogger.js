...
export function redactCredentialPaths(url) {
  if (typeof url !== "string") return url;
  return url
    .replace(/(\/preview\/)[A-Za-z0-9_-]{40,100}/g, "$1[REDACTED]")
    .replace(/(\/public\/prospects\/preview\/)[A-Za-z0-9_-]{40,100}/g, "$1[REDACTED]");
}

function getOrAssignRequestId(req, res) {
  const incoming = req.headers[REQUEST_ID_HEADER];
  // Do not reflect arbitrary header bytes into logs and response headers.
  // Accept common UUID/trace formats, but cap length and reject whitespace.
  const id =
    ...
    serializers: {
      req(req) {
        return {
          id: req.id,
          method: req.method,
          url: redactCredentialPaths(req.url),
