...
let outreachEmailQueue = null;

export function getOutreachEmailQueue() {
  if (outreachEmailQueue) return outreachEmailQueue;
  const connection = getRedisConnection();
  if (!connection) return null;
  outreachEmailQueue = new Queue("sp-outreach-email", {
    connection,
    defaultJobOptions: {
      attempts: 3,
      backoff: { type: "exponential", delay: 30_000 },
...
export function getAllQueues() {
  const entries = [
    ...
    { name: "sp-outreach-email", getter: getOutreachEmailQueue },
  ];
