...
/** POST /api/v1/workspaces/:id/tech-stack/property_api/test — validate property API connection */
studioRouter.post(
  `${BASE}/workspaces/:id/tech-stack/property_api/test`,
  requireClientOwner,
  async (req, res, next) => {
    try {
      // Read the saved connection to get provider + encrypted API key
      ...
      if (!conn || conn.connectionStatus !== "connected" || !conn.metadataJson) {
        return res.status(400).json({
          ok: false,
          error: "NO_CONNECTION",
          message: "No property API connection configured. Save your API key first.",
        });
      }
      ...
      const providerMap = { attom: "attom", estated: "estated", rentcast: "rentcast" };
      if (!providerMap[providerName]) {
        return res.status(400).json({
          ok: false,
          error: "UNKNOWN_PROVIDER",
          message: `Unknown provider "${meta.provider}". Use attom, estated, or rentcast.`,
        });
      }

      const { decryptToken } = await import("../../lib/tokenCrypto.js");
      let apiKey;
      try {
        ...
        return res.status(400).json({
          ...
          message: "Could not decrypt stored API key. Please re-enter it.",
        });
      }

      // Try a test lookup with a well-known address
      const { attomProvider } = await import("../industry/providers/attom.provider.js");
      const { estatedProvider } = await import("../industry/providers/estated.provider.js");
      const { rentcastProvider } = await import("../industry/providers/rentcast.provider.js");

      const providers = { attom: attomProvider, estated: estatedProvider, rentcast: rentcastProvider };
      const provider = providers[providerName];

      const testAddress = { street: "1600 Pennsylvania Ave NW", city: "Washington", state: "DC", zip: "20500" };
      const result = await provider.lookupByAddress(testAddress, apiKey);

      if (result) {
        res.json({ ok: true, provider: providerName, message: "API key is valid. Test lookup succeeded.", sample: result });
      } else {
        res.json({ ok: false, provider: providerName, message: "API responded but returned no data. Verify your API key and plan." });
      }
    } catch (err) {
      next(err);
    }
