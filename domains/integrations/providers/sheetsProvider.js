...
async function refreshAccessToken(integrationId, config) {
  ...
  if (!res.ok) {
    ...
    if (isPermanent) {
      ...
      console.error(`[SHEETS_REFRESH] Permanent failure (${res.status}) integrationId=${integrationId}: ${body.slice(0, 300)}`);
    }
    throw new Error(`Google Sheets token refresh failed (${res.status}): ${body.slice(0, 300)}`);
  }
