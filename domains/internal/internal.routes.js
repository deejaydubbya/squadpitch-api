...
const EmailDraftSchema = z.object({ subject: z.string().max(240).optional(), textBody: z.string().max(40_000).optional(), htmlBody: z.string().max(100_000).optional(), body: z.string().max(40_000).optional(), sendingAccountId: z.string().optional() });
const OutreachTemplateSchema = z.object({ subject: z.string().min(1).max(240), textBody: z.string().min(1).max(40_000), htmlBody: z.string().min(1).max(100_000) });
const SendingAccountSchema = z.object({
  ...
  smtpSecure: z.boolean().optional(), smtpEncryption: z.enum(["STARTTLS", "SSL_TLS", "NONE"]).default("STARTTLS"), enabled: z.boolean().default(true), isDefault: z.boolean().default(false), hourlyLimit: z.coerce.number().int().min(1).max(1000).default(25), dailyLimit: z.coerce.number().int().min(1).max(10000).default(100), delaySeconds: z.coerce.number().int().min(1).max(3600).default(60),
...
internalRouter.patch(`${BASE}/agent-outreach/template`, requireAdminRole, async (req, res, next) => { try { const parsed = OutreachTemplateSchema.safeParse(req.body); if (!parsed.success) return validationError(res, parsed.error); res.json(await outreachService.updateOutreachTemplate(parsed.data)); } catch (err) { next(err); } });
...
internalRouter.post(`${BASE}/agent-outreach/prospects/:id/send`, requireAdminRole, async (req, res, next) => { try { res.status(202).json(await outreachService.queueOutreachEmail(req.params.id, req.body?.sendingAccountId)); } catch (err) { next(err); } });
